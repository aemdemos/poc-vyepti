 ...
 *   data-maps-config        — on .locator block, holds the Maps configuration value
 *
...
export default function decorate(block) {
  ...
  const mapsConfig = cells[0]?.textContent.trim() || '';
  const defaultRadius = cells[1]?.textContent.trim() || '25';

  block.textContent = '';
  if (mapsConfig) block.dataset.mapsConfig = mapsConfig;
  ...
  welcomeHeading.textContent = 'Find a provider';
  const welcomeText = document.createElement('h2');
  welcomeText.className = 'locator-welcome-text';
  welcomeText.textContent = 'Enter your location above to begin your search.';
